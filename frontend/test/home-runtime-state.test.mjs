...
test('evaluation page is reachable and uses workspace-scoped evaluation and performance APIs', () => {
  assert.match(router, /path:\s*'\/evaluations'/)
  assert.match(appView, /to="\/evaluations"/)
  assert.match(evaluationView, /\/evaluations\/gold/)
  assert.match(evaluationView, /\/agent-runs\/metrics/)
  assert.match(evaluationView, /candidateRunId/)
  assert.match(evaluationView, /baselineRunId/)
})
