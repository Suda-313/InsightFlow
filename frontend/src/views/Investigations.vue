<template>
  <div class="p-6 max-w-7xl mx-auto">
    ...
    <div v-else>
    <section v-if="riskQueue.length" class="mb-5 card p-5">
      <div class="flex items-center justify-between gap-3"><div><h2 class="font-semibold">风险优先队列</h2><p class="mt-1 text-xs text-slate-500">按告警生成时冻结的优先级排序；分数和依据仅用于辅助人工判断。</p></div><span class="text-xs text-slate-400">{{ riskQueue.length }} 项</span></div>
      <div class="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        <button v-for="item in riskQueue" :key="item.alertId" class="rounded-lg border border-slate-200 p-3 text-left hover:border-primary" @click="openRisk(item)">
          <div class="flex items-center justify-between gap-2"><span class="status-pill" :class="priorityClass(item.level)">{{ priorityText(item.level) }}</span><span class="text-sm font-semibold">{{ item.score }} 分</span></div>
          <p class="mt-2 text-sm font-medium">{{ item.issueName || item.issueKey }}</p><p class="mt-1 text-xs text-slate-500">{{ item.currentCount }} 条 · {{ item.reasons }}</p>
          <p class="mt-2 text-xs text-primary">查看并开始处理 →</p>
        </button>
      </div>
    </section>
    <div v-if="!cases.length" class="card py-16 text-center"><SearchCheck class="mx-auto w-11 h-11 text-slate-300" /><h2 class="mt-4 font-semibold">暂无待办调查</h2><p class="mt-2 text-sm text-slate-500">新告警会在后台冻结证据后出现在这里。</p></div>
    ...
    <div v-else class="grid grid-cols-1 lg:grid-cols-3 gap-5">
      ...
      <section v-if="detail" class="lg:col-span-2 space-y-5">
        <article class="card p-5"><div class="flex items-center justify-between"><h2 class="font-semibold">证据快照</h2><span class="status-pill" :class="statusClass(detail.investigation.status)">{{ statusText(detail.investigation.status) }}</span></div>
          ...
          <div class="mt-4 flex flex-wrap items-center gap-2"><button v-if="detail.investigation.followUpStatus === 'awaiting_follow_up'" class="btn-primary text-sm" :disabled="followUpRunning" @click="startFollowUp">开始跟进</button><span v-else class="status-pill bg-blue-100 text-blue-700">跟进中</span><span v-if="detail.investigation.followUpStartedAt" class="text-xs text-slate-500">开始于 {{ formatTime(detail.investigation.followUpStartedAt) }}</span><span v-if="detail.investigation.followUpReminderAt" class="status-pill bg-amber-100 text-amber-700">待跟进（已提醒）</span></div>
        ...
        <article class="card p-5"><h2 class="font-semibold">提交纠错候选</h2><p class="mt-1 text-xs text-slate-500">候选不会直接改写规则；Owner 会在复核依据后决定是否发布。</p><div class="mt-3 flex flex-col sm:flex-row gap-2"><select v-model="correctionKind" class="rounded-lg border border-slate-300 px-3 py-2 text-sm"><option value="ISSUE_ALIAS">主题别名</option><option value="RULE_CANDIDATE">规则候选</option></select><input v-model.trim="correctionContent" maxlength="2000" class="flex-1 rounded-lg border border-slate-300 px-3 py-2 text-sm" placeholder="写清问题、建议与可验证依据" /><button class="btn-secondary text-sm" :disabled="!correctionContent || correctionSubmitting" @click="submitCorrection">提交</button></div><p v-if="correctionMessage" class="mt-2 text-xs text-slate-500">{{ correctionMessage }}</p></article>
      </section>
    </div>
    </div>
  </div>
...
const cases = ref([]), riskQueue = ref([]), detail = ref(null), selectedId = ref(''), loading = ref(true), error = ref('')
...
async function load() {
  ...
  try { const [caseResponse, riskResponse] = await Promise.all([fetch(endpoint()), fetch(`${endpoint()}/risk-queue`)]); const [caseData, riskData] = await Promise.all([caseResponse.json(), riskResponse.json()]); if (!caseResponse.ok) throw new Error(caseData?.error?.message || '调查列表加载失败'); if (!riskResponse.ok) throw new Error(riskData?.error?.message || '风险队列加载失败'); cases.value = caseData; riskQueue.value = riskData; if (!selectedId.value && caseData[0]) await selectCase(caseData[0].id) } catch (exception) { error.value = exception.message || '调查列表加载失败' } finally { loading.value = false }
}

/** 队列项只携带告警 UUID；入队接口服务端幂等地复用已有调查，再进入统一详情页。 */
async function openRisk(item) {
  followUpRunning.value = true
  try { const response = await fetch(`${endpoint()}/alerts/${item.alertId}`, { method: 'POST' }); const data = await response.json(); if (!response.ok) throw new Error(data?.error?.message || '风险调查创建失败'); await load(); await selectCase(data.id) } catch (exception) { error.value = exception.message || '风险调查创建失败' } finally { followUpRunning.value = false }
}
...
/** 开始跟进只记录首位响应人的时间，不派单、不抢占，也不改变调查取证状态。 */
async function startFollowUp() {
  followUpRunning.value = true
  try { const response = await fetch(`${endpoint()}/${selectedId.value}/follow-up`, { method: 'POST' }); const data = await response.json(); if (!response.ok) throw new Error(data?.error?.message || '开始跟进失败'); if (detail.value) detail.value.investigation = data; cases.value = cases.value.map(item => item.id === data.id ? data : item) } catch (exception) { error.value = exception.message || '开始跟进失败' } finally { followUpRunning.value = false }
}
...
function priorityText(level) { return ({ P0: 'P0 立即关注', P1: 'P1 高优先级', P2: 'P2 常规处理', P3: 'P3 持续观察' })[level] || level }
function priorityClass(level) { return ({ P0: 'bg-red-100 text-red-700', P1: 'bg-orange-100 text-orange-700', P2: 'bg-amber-100 text-amber-700', P3: 'bg-slate-100 text-slate-600' })[level] || 'bg-slate-100 text-slate-600' }
