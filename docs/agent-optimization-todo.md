...
- [ ] 实现异常调查工作流：异常原因、贡献主题、时间拐点、关联版本或活动、样本反馈。
- [ ] 实现告警触发后的异步自动调查：汇总趋势、样本、版本或活动关联，生成待人工确认的调查卡片，不自动修改业务状态。
- [ ] 实现提案式告警处置：Agent 只能建议确认、忽略、合并、分派或关闭；前端展示证据与影响范围，人工确认后由 Command Service 执行。
- [ ] 为写操作补齐角色权限、幂等键、变更预览、操作审计和可撤销记录；不向模型暴露原始数据库写权限。
- [ ] 支持人工纠错：修正主题、风险和结论，并沉淀为别名、规则或评测样本。
- [ ] 将人工纠错接入评测闭环：规则、Prompt 或知识库变更前后自动比较金标结果，质量退化时禁止发布。
...
P3 已完成。下一项正式开发工作为 P4 的“业务闭环与协作能力”：先完成告警自动调查卡片和人工确认的提案式处置，再引入用户、组织成员与角色权限；不向模型开放直接写库能力。
