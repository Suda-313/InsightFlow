...
### 2026-07-31：可解释风险优先队列与最小主动响应闭环

- **背景或现象：** 原系统可以生成告警、异步取证和人工提案，但运营人员缺少可解释的风险先后顺序，也无法识别告警是否已被实际接住。
- **根因：** 告警仅保存异常事实，未在触发时冻结面向运营的优先级依据；调查状态又混合了异步取证进度和人工响应事实，若直接加入派单或通知会扩大为工单系统。
- **方案与取舍：** 告警创建后以 `risk:v1` 冻结 P0-P3、分数和原因，队列只读取快照；响应闭环独立记录“等待跟进/跟进中”、首位响应人和时间。超过 SLA 只写一次站内提醒，不做责任人派发、外部通知或升级链路。
- **实现：** 新增风险优先级服务、快照实体/迁移、事件监听和工作区隔离队列 API；调查中心展示等级、分数、原因并可幂等进入调查。新增“开始跟进”命令、审计记录、SLA 定时提醒与三份 Flyway 迁移，前端展示跟进和提醒状态。
- **验证：** 风险、快照、队列、跟进和提醒聚焦测试 10/10 通过；迁移契约测试 8/8 通过；调查中心前端状态测试 5/5 通过，`npm run build` 通过。全量 Maven 测试为 461 无失败、2 个既存 RAG 临时目录清理错误（Windows `AccessDeniedException`）。
- **沉淀：** 优先级必须作为告警产生时的可追溯业务事实保存，不能在列表页随策略变化重算；人机闭环首期先记录“是否接住”，再基于真实漏处理数据判断是否需要复杂协同能力。
