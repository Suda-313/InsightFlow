    ...
    public void run(UUID taskPublicId, String workerId) {
        ...
        try {
            ...
            if (investigation == null) {
                completionService.fail(taskPublicId, workerId, "INVESTIGATION_CASE_NOT_FOUND", "调查卡片不存在或不属于当前工作区");
                return;
            }
            ...
            if (evidenceSnapshotRepository.existsByInvestigationCaseIdAndWorkspaceId(
                ...
                completionService.complete(taskPublicId, workerId, evidenceSnapshotRepository
            ...
            Alert alert = alertRepository.findById(investigation.getAlertId())
                    ...
                    .orElse(null);
            if (alert == null) {
                completionService.fail(taskPublicId, workerId, "ALERT_NOT_FOUND", "调查触发告警不存在或不属于当前工作区");
                return;
            }
            ...
            completionService.complete(taskPublicId, workerId, snapshots.size());
        } catch (Exception exception) {
            LOGGER.error("调查任务执行失败, task_id={}", taskPublicId, exception);
            completionService.fail(taskPublicId, workerId, "INVESTIGATION_FAILED", "调查证据装配失败，请稍后重试");
        }
    }
}
